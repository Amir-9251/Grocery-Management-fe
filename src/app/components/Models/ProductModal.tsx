...
import { GetCategoriesApi } from '../Pages/Category/api';
...
const ProductModal = ({
    ...
    const handleCategoryChange = (selectedOption: Option | null) => {
        ...
        if (updatedProduct && setUpdatedProduct) {
            setUpdatedProduct(prev => ({ ...prev, categoryId: categoryId }));
        }
    }
        ...
        <Dialog.Root open={open} onOpenChange={handleOpen}>
            ...
            <Dialog.Portal>
                ...
                <Dialog.Content className='fixed z-50 left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-xl p-8 w-full max-w-3xl'>
                    <Dialog.Title className='text-xl font-bold mb-4'>{checked === 'create' ? 'Add Product' : 'Update Product'}</Dialog.Title>
                    <div className="border border-slate-100 rounded-2xl px-6 py-10 max-h-[80vh] overflow-y-auto [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-gray-300 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-gray-400">
                        <form onSubmit={checked === 'create' ? handleAddProduct : handleUpdateProduct} className="flex flex-col gap-4">
                            <div className='grid grid-cols-2 gap-x-16 gap-y-4'>
                                ...
                                <div>
                                    ...
                                    <StyledSelect
                                        options={Categories}
                                        name='Categories'
                                        value={formData.category ? { value: formData.category.name, label: formData.category.name } : null}
                            ...
                            <div className="h-32 w-72 flex flex-col gap-2 items-center mt-4 border border-dashed border-slate-300 rounded-xl justify-center hover:border-slate-400 transition-colors duration-200 cursor-pointer">
                                <IconPlus className="text-2xl text-slate-400" />
                                <span className=" text-gray-400 ">Upload Invoice</span>
                            </div>
